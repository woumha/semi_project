...
public class tokenCheckOkAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// save는 세션에 mail과 생성한 토큰이 저장되어 있음
		userTokenDTO save = (userTokenDTO)session.getAttribute("user");
		String userMail = request.getParameter("email");
		int inPutToken = Integer.parseInt(request.getParameter("inputToken").trim());
		...
		if(save.getEmail().equals(userMail)) { // 만약 다른 이용자가 타이용자의 토큰을 입력시 오류 방지
